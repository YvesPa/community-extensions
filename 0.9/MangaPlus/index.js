...
var source = (() => {
  ...
  function langPopup(errorResult, lang) {
    return errorResult?.popups?.find((popup) => (popup.language ?? "ENGLISH" /* ENGLISH */) === lang) || null;
  }
  var Title = class {
  ...
  var MangaPlusSettingForm = class extends import_types2.Form {
      ...
      return [
        (0, import_types2.Section)("content_settings", [
          (0, import_types2.SelectRow)("languages", {
            ...
            onValueChange: Application.Selector(this, "setLanguages")
          }),
          (0, import_types2.ToggleRow)("split_images", {
            title: "Split Images",
            value: getSplitImages() === "yes",
            onValueChange: Application.Selector(this, "setSplitImages")
          }),
          (0, import_types2.SelectRow)("image_resolution", {
            ...
            onValueChange: Application.Selector(this, "setResolution")
          })
        ]),
    ...
    async resetSettings() {
      await Application.setState(["ENGLISH" /* ENGLISH */], "languages");
      await Application.setState("yes", "split_images");
      await Application.setState("high", "image_resolution");
    }
  };
  ...
  var MangaPlusSource = class {
    // eslint-disable-next-line @typescript-eslint/no-empty-function
    constructor() {
      this.globalRateLimiter = new import_types3.BasicRateLimiter("rateLimiter", { numberOfRequests: 10, bufferInterval: 1, ignoreImages: true });
    }
    async initialise() {
    ...
    async getChapterDetails(chapter) {
      ...
      const response = (await Application.scheduleRequest(request))[1];
      const result = JSON.parse(Application.arrayBufferToUTF8String(response));
      if (result.success === void 0) {
        throw new Error(langPopup(result.error, "ENGLISH" /* ENGLISH */)?.body ?? "Unknown error");
      }
    ...
    async getFeaturedTitles(section, metadata) {
      const request = {
    ...
    async getPopularTitles(section, metadata) {
      const request = {
    ...
    async getSearchResults(query, metadata) {
      ...
      for (const item of results ?? []) {
        ...
        const author = item.author;
        const image = item.portraitImageUrl;
        if (!mangaId || !title2 || collectedIds.includes(mangaId))
          continue;
        titles.push({
          mangaId,
          title: title2,
    ...
    async registerDiscoverSections() {
      Application.registerDiscoverSection(
        ...
        Application.Selector(this, "getFeaturedTitles")
      );
      Application.registerDiscoverSection(
        {
          id: "popular",
      ...
      Application.registerDiscoverSection(
        ...
        Application.Selector(this, "getLatestUpdates")
      );
    }
    /* TODO ?
  ...
  // src/MangaPlus/main.ts
  var MangaPlus = new MangaPlusSource();
